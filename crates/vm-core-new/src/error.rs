...
pub enum VmError {
    ...
    /// Attempted to read from an uninitialized memory input region.
    #[error("read from uninitialized memory input at {ptr:#x}")]
    UninitializedMemoryInput { ptr: u32 },

    /// Memory region already marked as input.
    #[error("memory region at {ptr:#x} already marked as input")]
    DuplicateMemoryInput { ptr: u32 },

    /// No memory input registered for this pointer.
    #[error("no memory input at {ptr:#x}")]
    UnknownMemoryInput { ptr: u32 },

    /// Memory input length mismatch.
    #[error("memory input length mismatch at {ptr:#x}: expected {expected}, got {got}")]
    MemoryInputLengthMismatch {
        ptr: u32,
        expected: usize,
        got: usize,
    },
