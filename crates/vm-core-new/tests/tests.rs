...
use mpz_vm_core_new::{Module, Param, ideal::Instance, value::Value};
...
fn test_aes() {
    ...
    block_on(join(
        async {
            ...
            vm_a.mark_private(key_ptr, 16).unwrap();
            vm_a.mark_blind(msg_ptr, 16).unwrap();

            // Write our private key
            vm_a.write_private(key_ptr, &key).unwrap();
        ...
        async {
            ...
            vm_b.mark_blind(key_ptr, 16).unwrap();
            vm_b.mark_private(msg_ptr, 16).unwrap();

            // Write our private msg
            vm_b.write_private(msg_ptr, &msg).unwrap();
...
fn test_println() {
    ...
    // test_print has no private/blind values, so it should complete synchronously
    let out = vm.call_sync(func_idx as u32, vec![]).unwrap();
    assert!(out.is_none());
}

#[test]
fn test_json_parse() {
    static JSON_FIXTURE: &[u8] = include_bytes!("../../../crates/vm-core-tests/fixtures/sample.json");
    ...
    block_on(join(
        async {
            ...
            vm_a.mark_private(json_ptr, json_len).unwrap();
            vm_a.write_private(json_ptr, &json_data).unwrap();
        ...
        async {
            ...
            vm_b.mark_blind(json_ptr, json_len).unwrap();
