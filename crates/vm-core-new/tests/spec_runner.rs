...
use ir::{self, Module};
use mpz_vm_core_new::{Param, VmError, ideal::Instance, value::Value};
use wast::{
...
impl VmPair {
    fn new(module: Module) -> Result<Self, String> {
        let vm_a = Instance::new_local(module.clone()).map_err(|e| format!("{:?}", e))?;
        let vm_b = Instance::new_local(module.clone()).map_err(|e| format!("{:?}", e))?;
        Ok(Self { vm_a, vm_b, module })
    }

    /// Reset VMs to initial state (after a trap corrupts them).
    fn reset(&mut self) -> Result<(), String> {
        self.vm_a = Instance::new_local(self.module.clone()).map_err(|e| format!("{:?}", e))?;
        self.vm_b = Instance::new_local(self.module.clone()).map_err(|e| format!("{:?}", e))?;
        Ok(())
    }
    ...
    fn execute(
        ...
        let result_a = self
            .vm_a
            .call_sync(func_idx, args_a)
            .map_err(SkipReason::from)?;
        let result_b = self
            .vm_b
            .call_sync(func_idx, args_b)
            .map_err(SkipReason::from)?;
